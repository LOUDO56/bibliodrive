        ...
        if (isset($_SESSION["search"])) {
            echo '
                    <div class="retour-detail">
                        <a href="search?auteur=' . $_SESSION["search"] . '#livre_' . $info_livre->nolivre . '">← Retour</a>
                    </div>
                    ';
        }
        ?>
        <p><b>Auteur:</b>
            <?php echo $info_livre->nom . " " . $info_livre->prenom; ?>
        </p>
        <p><b>ISBN13:</b>
            <?php echo $info_livre->isbn13; ?>
        </p>
                ...
                if (isset($user_mel->mel) && $user_mel->mel == $_SESSION["email"]) {
                    ...
                    if (!$emprute) {
                        if (in_array($_GET["livre"], $_SESSION["panier"])) {
                            echo '<a href="basketmanager?retirer=true&nolivre=' . $_GET["livre"] . '&redirect=detail?livre=' . $_GET["livre"] . '" class="button-general retirer-panier">Retirer du panier</a>';
                        } else {
                            if ((count($_SESSION["panier"]) + $req->rowCount()) == 5) {
                                ...
                                echo '<a href="basketmanager?ajout=true&nolivre=' . $_GET["livre"] . '&redirect=detail?livre=' . $_GET["livre"] . '" class="button-general ajout-panier">Ajouter au panier</a>';
                            }
                        }
                    }
                }
